...
class TestFollowAPI:
    ...
    def test_follow_get(self, user_client, user, follow_1, follow_2, follow_3):
        ...
        assert len(test_data) == Follow.objects.filter(author__username=user.username).count(), (
        ...
        assert 'author' in test_group, (
            'Проверьте, что добавили `author` в список полей `fields` сериализатора модели Follow'
        )
        ...
        assert test_group['author'] == follow.author.username, (
            ...
            'в поле `author` должен быть `username`'
        )
    ...
    def test_follow_search_filter(self, user_client, follow_1, follow_2,
        ...
        assert len(response.json()) == follow_user.filter(author=user_2).count(), (
        ...
        assert len(response.json()) == follow_user.filter(author=another_user).count(), (
            'Проверьте, что при GET запросе с параметром `search` на `/api/v1/follow/` '
            'возвращается результат поиска по подписке'
        )
